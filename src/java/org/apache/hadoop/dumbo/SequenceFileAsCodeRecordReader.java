...
/**
 * An record reader that converts sequence file records to Dumbo code.
 */
public class SequenceFileAsCodeRecordReader implements RecordReader<Text, Text> {
  ...
  public SequenceFileAsCodeRecordReader(Configuration conf, FileSplit split)
  throws IOException {
  ...
  public synchronized boolean next(Text key, Text value) throws IOException {
    if (!sequenceFileRecordReader.next(innerKey, innerValue)) return false;
    key.set(CodeUtils.writableToCode(innerKey));
    value.set(CodeUtils.writableToCode(innerValue));
    return true;
  }
